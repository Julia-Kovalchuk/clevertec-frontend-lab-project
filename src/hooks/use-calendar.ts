...
import { createDate, createMonth, getMonthesNames, getMonthNumberOfDays, getWeekDaysNames } from '../utils/date';
...
export const useCalendar = ({ locale = 'default', selectedDate: date, firstWeekDayNumber = 2 }: UseCalendarParams) => {
  ...
  const onClickArrow = (direction: 'up' | 'down') => {
    ...
    if (mode === 'monthes' && direction === 'down') {
